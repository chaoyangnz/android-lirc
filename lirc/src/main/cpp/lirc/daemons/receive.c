...
lirc_t lirc_t_max(lirc_t a,lirc_t b)
{
	return(a>b ? a:b);
}

void set_pending_pulse(lirc_t deltap)
{
	LOGPRINTF(5, "pending pulse: %lu", deltap);
	rec_buffer.pendingp=deltap;
}

void set_pending_space(lirc_t deltas)
{
...
lirc_t get_next_pulse(lirc_t maxusec)
{
	lirc_t data;
...
int sync_pending_pulse(struct ir_remote *remote)
{
...
int sync_pending_space(struct ir_remote *remote)
{
...
int expectone(struct ir_remote *remote,int bit)
{
...
int expectzero(struct ir_remote *remote,int bit)
{
	if(is_biphase(remote))
	{
		int all_bits = bit_count(remote);
...
lirc_t sync_rec_buffer(struct ir_remote *remote)
{
	int count;
...
int get_gap(struct ir_remote *remote,lirc_t gap)
{
	lirc_t data;
...
int get_repeat(struct ir_remote *remote)
{
